...
var unregisterCmd = &cobra.Command{
	Use:   "unregister",
	Short: "Unregister a broker",
	Long:  `Unregisters a broker from the persistent store.`,
	Args:  cobra.ExactArgs(1),
	Run: func(cmd *cobra.Command, args []string) {
		brokerId := args[0]
		repo := brokerrepo.NewSqliteRepo("acectl.db")
		srv := brokersrv.New(repo)
		err := srv.Unregister(brokerId)
		if err != nil {
			fmt.Println(err.Error())
			return
		}
		fmt.Println("successfully unregistered broker [" + brokerId + "]")
	},
}
